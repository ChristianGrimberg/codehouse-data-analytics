...
if(Invoke-Command -ScriptBlock $scriptSystemBlock) {
    switch ($Task) {
        ...
        Artifact {
            [String] $sqlArtifactScriptPath = Join-Path -Path $scriptsPath -ChildPath "New-SqlServerArtifacts.ps1"
            [String] $sqlArtifactScriptToRun = "{0} -ModulesPath {1} -FunctionsPath {2}" -f $sqlArtifactScriptPath, $modulesPath, $scriptsPath
            $scriptSqlArtifactBlock = [System.Management.Automation.ScriptBlock]::Create($sqlArtifactScriptToRun)

            if(Invoke-Command -ScriptBlock $scriptSqlArtifactBlock) {
