...
try {
    ...
    if ($sqlFilesToLint.Count -gt 0) {
        ...
        "Starting comprehensive SQL Server linting analysis..." | Out-Host
        "Found {0} SQL files to analyze" -f $sqlFilesToLint.Count | Out-Host
        "" | Out-Host

        # Check for available linting tools
        $hasToolInstalled = $false
        $toolsChecked = @()

        # Check for SQLFluff (Python-based)
        $sqlFluffAvailable = $false
        try {
            $sqlFluffCheck = & python -m sqlfluff --version 2>&1
            if ($LASTEXITCODE -eq 0) {
                $sqlFluffAvailable = $true
                $hasToolInstalled = $true
        ...
        # Check for TSqlLint (dotnet tool)
        $tsqllintAvailable = $false
        try {
            $tsqllintCheck = & dotnet tool list -g 2>&1 | Select-String -Pattern "tsqllint"
            if ($null -ne $tsqllintCheck) {
                $tsqllintAvailable = $true
                $hasToolInstalled = $true
        ...
        if (-not $hasToolInstalled) {
            $Host.UI.RawUI.ForegroundColor = "Yellow"
            "WARNING: No SQL linting tools found (SQLFluff, TSqlLint)." | Out-Host
            "Performing basic built-in SQL validation instead..." | Out-Host
            "" | Out-Host
            ...
            foreach ($sqlFile in $sqlFilesToLint) {
                "Linting: {0}" -f $sqlFile.Name | Out-Host
                ...
                if ($sqlFluffAvailable) {
                    try {
                        ...
                        if ($LASTEXITCODE -eq 0 -or $LASTEXITCODE -eq 1) {
                            # Parse JSON output
                            try {
                                $sqlFluffResults = $sqlFluffOutput | ConvertFrom-Json

                                foreach ($violation in $sqlFluffResults) {
                                    if ($violation.violations) {
                                        foreach ($v in $violation.violations) {
                                            $severity = "Information"
                                            if ($v.rule_code -like "L*") {
                                                $severity = "Information"
                                            }
        ...
        # Installation guidance if no tools found
        if (-not $hasToolInstalled) {
            "" | Out-Host
            $Host.UI.RawUI.ForegroundColor = "Yellow"
            "RECOMMENDATION: Install SQL linting tools for more comprehensive analysis:" | Out-Host
            "  - SQLFluff: pip install sqlfluff" | Out-Host
